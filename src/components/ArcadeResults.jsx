...
export default function ArcadeResults({ onRestart, onExit }) {
  ...
  return (
    <div className="absolute inset-0 z-[100] bg-zinc-950/95 backdrop-blur-md flex flex-col items-center justify-center p-6">
      ...
      <div className="glass-panel backdrop-blur-md animate-boss-breathe w-full max-w-sm rounded-2xl p-6 border border-cyan-500/30 bg-zinc-950/60 flex flex-col gap-4">

        {/* ── Header ── */}
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1">
            <p className="font-mono text-[10px] uppercase tracking-widest text-cyan-400/50 mb-1">
            ...
            <p className={`font-mono text-[10px] uppercase tracking-widest mt-1 ${rank.color}`}>
              {rank.title}
            </p>
          </div>
          {/* ── Letter Grade ── */}
        ...
        <div className="glass-panel border-2 border-cyan-500/60 bg-cyan-500/5 p-5 flex flex-col items-center rounded-lg relative">
          {isPersonalBest && (
            <span className="absolute top-2 right-2 font-mono text-[8px] uppercase tracking-widest text-amber-400 border border-amber-400/50 bg-amber-400/10 px-1.5 py-0.5 rounded animate-pulse">
              PERSONAL BEST
            </span>
          )}
          <span className="font-mono text-[10px] uppercase tracking-widest text-cyan-400/60 mb-2">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="glass-panel border border-fuchsia-500/30 bg-zinc-900/40 p-4 flex flex-col items-center rounded-lg">
            <span className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 mb-1">
              Keystrokes
            </span>
